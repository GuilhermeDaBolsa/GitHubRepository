...
public class YSlidingBar extends YVisibleScene {
    ...
    private boolean nextFrame(int frame_index1, int frame_index2, double pX, double pY){
        ...
        if(minor_distance < YMath.yHypotenuse(pX - PATH_POINT_LIST.get(CURRENT_POSITION_INDEX).getX(), pY - PATH_POINT_LIST.get(CURRENT_POSITION_INDEX).getY())){
    ...
    private double distanceToNextPoint(int frame_index, double pX, double pY){
        return YMath.yHypotenuse(pX - PATH_POINT_LIST.get(frame_index).getX(), pY - PATH_POINT_LIST.get(frame_index).getY());
    }
}