...
public class Caixa extends InteractiveObject {
    ...
    /**
     * Método para mudar o tamanho da caixa.
     * @param escalaX Fator de escala da largura.
     * @param escalaY Fator de escala da altura
     */
    public void mudar_tamanho_caixa(double escalaX, double escalaY){
        if(escalaX != Double.NaN)
            caixa.setScaleX(escalaX);
        if(escalaY != Double.NaN)
            caixa.setScaleY(escalaY);
    }
    ...
    /**
     * Adiciona um conteúdo a caixa (um Nodo).
     * @param conteudo Contúdo a ser adicionado.
     */
    public void adicionar_conteudo(Node conteudo) {
        conteudo_caixa.add(conteudo);
        container.getChildren().add(conteudo);
    }
    
    /**
       * Adiciona vários conteúdos a caixa (varios Nodos).
       * @param conteudo Contúdos a serem adicionados.
    */
    public void adicionar_conteudo(Node... conteudo) {
        for (int i = 0; i < conteudo.length; i++) {
            adicionar_conteudo(conteudo[i]);
        }
    }
